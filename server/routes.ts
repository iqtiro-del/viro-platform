...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment/callback", async (req, res) => {
    try {
      ...
      // Handle Spaceremit callback
      // The callback typically contains payment_id, status, amount, etc.
      if (data.payment_id && data.status) {
        console.log('[SPACEREMIT CALLBACK]', {
        ...
        // For callbacks, we can auto-verify and credit if needed
        // However, we primarily rely on the frontend verification flow
        // This callback serves as a backup notification
      }
      
      res.json({ status: "received", message: "Callback processed successfully" });
    } catch (error: any) {
  ...
  // Spaceremit Payment Gateway - Get public key for frontend
  ...
  // Spaceremit Payment Gateway - Verify payment and credit wallet
  app.post("/api/payments/verify", async (req, res) => {
    try {
      const { userId, paymentCode, amount } = req.body;
      
      if (!userId || !paymentCode || !amount) {
      ...
      const verifyResponse = await fetch('https://spaceremit.com/api/v2/payment_info/', {
        method: 'POST',
        headers: {
      ...
      console.log("Spaceremit verification result:", JSON.stringify(verifyResult, null, 2));

      if (verifyResult.response_status !== 'success') {
        return res.status(400).json({ 
          error: "Payment verification failed", 
          details: verifyResult.message || "Invalid payment code" 
        });
      }

      const paymentData = verifyResult.data;
      
      // Validate payment status - must be completed/settled
      // status_tag: 'T' for test payments, 'C' for completed, 'P' for pending, 'F' for failed
      const statusTag = paymentData.status_tag;
      const paymentStatus = paymentData.status?.toLowerCase() || '';
      
      // Accept test payments (status_tag: 'T') for testing
      // Accept completed payments (status contains 'completed', 'paid', 'settled', or 'success')
      const isTestPayment = statusTag === 'T';
      const isCompletedPayment = 
        statusTag === 'C' || 
        paymentStatus.includes('completed') || 
        paymentStatus.includes('paid') || 
        paymentStatus.includes('settled') ||
        paymentStatus.includes('success');
      
      if (!isTestPayment && !isCompletedPayment) {
        console.log('[SPACEREMIT] Payment not completed:', {
          statusTag,
        ...
        return res.status(400).json({ 
          ...
          details: `Payment status: ${paymentData.status || 'unknown'}. Please wait for the payment to be processed.`
        });
      }
      
      const paidAmount = parseFloat(paymentData.buyer_payed_amount || paymentData.total_amount || '0');
      ...
      if (currency && currency !== 'USD') {
        console.log('[SPACEREMIT] Unexpected currency:', currency);
        // Still allow but log the discrepancy
      }

      // Apply 5% platform fee
      const feeRate = 0.05;
      const feeAmount = paidAmount * feeRate;
      const amountToCredit = paidAmount - feeAmount;

      // Check for duplicate payment (idempotency)
      const existingTransactions = await storage.getTransactionsByUser(userId);
      const duplicatePayment = existingTransactions.find(
        t => t.description && t.description.includes(paymentCode) && t.status === 'completed'
      );
      ...
      await storage.createTransaction({
        ...
        description: `Online payment via Spaceremit - ${paymentCode}`,
        status: 'completed'
      });

      console.log('[SPACEREMIT DEPOSIT]', {
        userId,
        username: user.username,
        paidAmount,
        feeAmount: feeAmount.toFixed(2),
        amountCredited: amountToCredit.toFixed(2),
        newBalance: newBalance.toFixed(2),
        paymentCode
      });
